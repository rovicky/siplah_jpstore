class BaseUrl {
  static String base = "https://siplah.jpstore.id/api/";
  static String baseAssets = "https://siplah.jpstore.id/assets/";
  static String rjUrl = "https://pro.rajaongkir.com/api/";
  static String baseImage = "https://firebasestorage.googleapis.com/v0/b/lelang-ukk.appspot.com/o/icon%2F1024px-No_image_available.svg.png?alt=media&token=5e988671-0d63-4296-9f97-69d055a4396f";
  static String placeholder = "src/image/no_image_placeholder.png";
  static Headers headers = Headers();
}

class Headers{
  final String contentTypeJson = "application/json";
  final String contentTypeurlx = "application/x-www-form-urlencoded";
  final String apiApp = "siplah_jpmall.id";
  final String apiKey = "4P1_7Pm411_51p114h";
  final String apiToken = "1693122b60431ef96822c5433a7531eef633be5a";
  final String rjKey = "b26afd088cb93037afed86af9a5e6d4a";
  final String courierProvider = "jne:jnt:pos:tiki";
}

//1693122b60431ef96822c5433a7531eef633be5a